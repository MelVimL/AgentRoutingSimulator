...
class SimulationAPI():
    ...
    def set_time_step(simulation_id: int, time_step) -> None:
        with get_session() as session:
            with session.begin():
                statement = update(Simulation).where(
                    Simulation.id == simulation_id).values(current_time_step=time_step)
...
class StatsAPI:
    ...
    def add_stat(simulation_id: int, stat_type_id: int, time_step: int, value):
        stat = Stat(simulation_id=simulation_id, stat_type_id=stat_type_id, step=time_step, value=value)
        with get_session() as session, session.begin():
            session.add(stat)
    ...
    def get_stats(simulation_id: int, stat_type_id: int,):
        with get_session() as session, session.begin():
            statement = select(Stat).where(Simulation.id == simulation_id).where(StatType.id == stat_type_id).order_by(Stat.step)
            return session.execute(statement).all()